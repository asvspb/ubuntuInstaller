...
show_menu() {
    ...
    echo -e "${C_CYAN}         🛠️  UBUNTU WORKSTATION INSTALLER & POST-INSTALL ENGINE       ${C_RESET}"
    echo -e "${C_BOLD}======================================================================${C_RESET}"
    echo -e "Выберите сценарий настройки:"
    echo -e "  ${C_BOLD}[1]${C_RESET} 🚀 ${C_GREEN}ПОЛНАЯ УСТАНОВКА «ПОД КЛЮЧ»${C_RESET} (Шаги 1, 2, 3, 4 + Dotfiles)"
    echo -e "  --------------------------------------------------------------------"
    echo -e "  ${C_BOLD}[2]${C_RESET} ⚙️  Шаг 1: Базовая система, sudo, Chrome, Telegram"
    echo -e "  ${C_BOLD}[3]${C_RESET} 🐳 Шаг 2: Docker, Docker Compose, Lazydocker"
    ...
    echo -e "  ${C_BOLD}[6]${C_RESET} 👤 Шаг 5: Развертывание конфигов и dotfiles (\$USER -> \$HOME)"
    echo -e "  --------------------------------------------------------------------"
    echo -e "  ${C_BOLD}[7]${C_RESET} 🌐 ZeroTier клиент и управление VDS-сервером"
    echo -e "  ${C_BOLD}[8]${C_RESET} 🔒 v2rayA прокси и локальный SOCKS5-туннель"
    ...
    case "$choice" in
        ...
        6) bash "$SCRIPTS_DIR/deploy-dotfiles.sh" ;;
        7)
