...
Для обеспечения максимальной надежности теперь используются интерактивные оболочки (`bash -ic` или `zsh -ic`), которые напрямую вызывают функции `agy1` и `agy2`.
...
| 2 | `2agy` имел дублирующуюся и хрупкую логику запуска `dbus` и переменных | Функция полностью переписана для прямого вызова существующих и проверенных функций `agy1`/`agy2` через `bash/zsh -ic` |
