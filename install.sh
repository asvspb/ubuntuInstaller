...
# Предварительное сканирование аргументов для определения флага --dry-run
# Это необходимо для корректной работы DRY_RUN в lib.sh
UBUNTU_INSTALLER_DRY_RUN="false"
for arg in "$@"; do
  if [ "$arg" = "--dry-run" ]; then
    UBUNTU_INSTALLER_DRY_RUN="true"
    break
  fi
done
export UBUNTU_INSTALLER_DRY_RUN
...
run_roles() {
  ...
  for i in $(seq 0 $((roles_count - 1))); do
    ...
    if [ "$is_critical_role" = true ] && [ "$DRY_RUN_FLAG" = false ]; then
      ...
      if ! create_snapshot "$snapshot_desc"; then
        log "WARN" "Не удалось создать снапшот перед установкой критичной роли: $role_name (установка продолжается)"
      fi
    fi
...
# Основная функция
main() {
  # Проверяем права root только если не dry-run
