...
# Функция для определения пути к лог-файлу с фолбэком
get_log_file_path() {
  local config_log_file="${UBUNTU_INSTALLER_LOG_FILE:-/var/log/ubuntuInstaller/install-$(date +%Y-%m-%d).log}"

  # Проверяем, можем ли мы писать в директорию лог-файла из конфигурации
  local log_dir=$(dirname "$config_log_file")
  if [ -w "$log_dir" ] || sudo -n test -w "$log_dir" 2>/dev/null; then
    # Если можем писать в указанную директорию, используем путь из конфигурации
    echo "$config_log_file"
  else
    # Если нет прав на запись в указанную директорию, используем фолбэк
    echo "$HOME/.cache/ubuntuInstaller/logs/install-$(date +%Y-%m-%d).log"
  fi
}
...
if [ ! -d "$log_dir" ]; then
  if [ -w "$(dirname "$log_dir")" ] || sudo -n true 2>/dev/null; then
...
log() {
  ...
  *)
  ...
  # Запись в лог-файл
  # Проверяем, можем ли мы писать в лог-файл напрямую
  if [ -w "$(dirname "$LOG_FILE")" ]; then
    # Если директория доступна для записи, пишем напрямую
    echo "$log_entry" >>"$LOG_FILE"
  elif sudo -n test -w "$(dirname "$LOG_FILE")" 2>/dev/null; then
    # Если у нас есть права sudo, используем их
    echo "$log_entry" | sudo tee -a "$LOG_FILE" >/dev/null
  else
    # Если нет прав, выводим предупреждение
    echo "Не удалось записать в лог-файл: $LOG_FILE (нет прав на запись)"
  fi
}
...
ensure_snap_pkg() {
  ...
  if ! sudo snap list | grep -q "^$package "; then
...
create_snapshot() {
  ...
  # Проверяем поддержку снапшотов
  local snapshot_support
  snapshot_support=$(check_snapshot_support)
  if [ "$snapshot_support" = "none" ]; then
    log "WARN" "Система снапшотов не найдена, невозможно создать снапшот: $description"
    return 1
  fi

  # Создание снапшота в зависимости от доступной системы
  if [ "$snapshot_support" = "timeshift" ]; then
    ...
    if [ $? -eq 0 ]; then
      ...
      log "WARN" "Не удалось создать снапшот с помощью Timeshift: $description"
      return 1
    fi
  else
    log "WARN" "Поддержка снапшотов типа $snapshot_support не реализована: $description"
    return 1
  fi
}
